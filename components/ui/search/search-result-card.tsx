		...
		<div className="min-h-32 xl:min-h-72 bg-primary-foreground rounded-sm border cursor-pointer group hover:border-accent hover:bg-[#f3fdf3]">
			<div className="flex justify-between items-center gap-2 px-4 pt-2.5 pb-1.5 xl:px-20 xl:pt-9 xl:pb-6">
				<div className="flex flex-col gap-5">
					<div className="flex items-center gap-5">
						...
						<div className="xl:space-y-2">
							...
							<p className="text-xs xl:text-base font-bold text-accent">
								{job.company}
							</p>
						</div>
					</div>
					<div className="flex items-center gap-3">
						<div className="flex items-center gap-1.5">
							...
							/>
							<span className="text-xs xl:text-base text-secondary leading-6">
								{job.location}
							</span>
						</div>
						<div className="flex items-center gap-1.5">
				...
				<div className="self-start">
					<button
						type="button"
						className={`bg-transparent border ${isFavorited ? "border-accent" : "border-muted/50"} rounded-full w-8 h-8 xl:w-14 xl:h-14 flex items-center justify-center cursor-pointer group/fav hover:border-accent`}
						onClick={() => setIsFavorited(!isFavorited)}
					>
						<HeartIcon
							className={`max-xl:w-3.5 ${isFavorited ? "fill-accent stroke-accent" : "fill-muted stroke-muted"} group-hover/fav:fill-accent group-hover/fav:stroke-accent`}
							size={20}
						/>
					</button>
				</div>
			...
			<div className="text-secondary text-[0.625rem] px-4 py-1.5 xl:text-base xl:px-20 xl:py-6">
				{job.categories.join("/")}
			</div>
		</div>
	);
